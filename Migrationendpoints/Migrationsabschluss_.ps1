# Filter  Batches
$batches = Get-MigrationBatch | where { $_.identity -match "Bezirk"}                     # Batches Filtern
$batches
...
# Notcompleted
$Notcompleted = $MigrationUser | ? { $_.status -ne "completed" }                        # MigrationsUSER nach Status filtern
$Notcompleted| Get-MigrationUser | ft status

# (2) Approve + ReSync +  Complete
$Notcompleted | % { set-MigrationUser $_.guid -ApproveSkippedItems }                         # APPROVE USER mit Status "Investigate"
$Notcompleted | % { set-MigrationUser $_.guid -SyncNow }                                     # SYNC     Now
$Notcompleted | % { set-MigrationUser $_.guid -CompleteAfter (get-date).ToUniversalTime() }  # COMPLETE Now

# NeedsApproval
$NeedsApproval = $MigrationUser | ? { $_.status -eq "NeedsApproval" }                        # MigrationsUSER nach Status filtern
